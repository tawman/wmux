...
async function cmdA2a(args) {
    ...
    switch (sub) {
        case 'send': {
            ...
            if (!to) {
                console.error('Usage: wmux a2a send --to <id> [--kind <kind>] <payload>');
                process.exit(1);
            }
            const from = getFlag(args, '--from') || self;
            if (!from) {
                console.error('No sender id. Pass --from <id> or run inside a wmux pane.');
                process.exit(1);
            }
        ...
        case 'poll': {
            ...
            if (!to) {
                console.error('No inbox id. Pass --to <id> or run inside a wmux pane.');
                process.exit(1);
            }
