 ...
 *
 * ONE art file feeds every size. That is a change from the three-tier setup
 * this script used to carry (issue #122: icon.svg / icon-small.svg /
 * icon-tiny.svg), which existed because the old gradient split-pane mark had
 * detail — `>_` prompts, a 58-unit W stroke, a 72-unit frame — that turned to
 * grey mush below 32px, and no amount of resampling fixes art that is too fine.
 *
 * The current mark is three solid bars on a plate, and it was measured rather
 * than assumed: rendering it down the whole ICO ladder and reading the
 * luminance profile across the middle of the mark gives a peak-to-valley
 * separation of 211/255 at EVERY size from 16px up, with each bar still
 * reaching full brightness and each gap still falling back to plate level.
 * There is nothing for a simplified variant to rescue, and three near-identical
 * SVGs is precisely the duplicated-asset trap that let a stale copy ship for
 * three releases in issue #137. If the art ever regains fine detail, restore
 * the tiers — the mechanism was only removed because this mark does not need it.
 *
...
/** Every entry Windows may ask for, from the tray up to the 256px shell tile. */
...
async function main() {
  ...
  const art = fs.readFileSync(path.join(ICONS, ART), 'utf-8');

  const entries = [];
  for (const size of ICO_SIZES) {
    entries.push({ size, png: await renderPng(win, art, size) });
    log(`  ${size}px`);
  }
  ...
  const master = await renderPng(win, art, 512);
  ...
  // Site favicon — also the apple-touch-icon, which is why it is 256 and not 32.
  const favicon = await renderPng(win, art, 256);
  fs.writeFileSync(path.join(ROOT, 'site', 'assets', 'favicon.png'), favicon);
  log('  wrote site/assets/favicon.png');

  // The orchestrator page needs its own copy. netlify.toml rewrites
  // plugin.wmux.org/* to /orchestrator/:splat, so a relative path there can
  // never climb out to site/assets/ — `../assets/` just normalises back to
  // /assets/ under that host and 404s, which is what it had been doing.
  // A second *generated* file is fine; a second hand-maintained one would not be.
  const orchAssets = path.join(ROOT, 'site', 'orchestrator', 'assets');
  fs.mkdirSync(orchAssets, { recursive: true });
  fs.writeFileSync(path.join(orchAssets, 'favicon.png'), favicon);
  log('  wrote site/orchestrator/assets/favicon.png');

  // The site's own header/hero logo. Generated here rather than hand-exported so
  // it cannot drift from the app icon the way a manual copy would.
  const siteLogo = await renderPng(win, art, 1024);
  fs.writeFileSync(path.join(ROOT, 'site', 'assets', 'wmux_logo.png'), siteLogo);
  log('  wrote site/assets/wmux_logo.png (1024px)');
